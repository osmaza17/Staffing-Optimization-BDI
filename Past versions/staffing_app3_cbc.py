import flet as ft
from pulp import * # Librería para programación lineal (Optimización)
import json
import os
...
def solve_model(data):
    """
    Construye y resuelve el modelo de optimización lineal usando PuLP.
    
    Inputs:
        data: Diccionario con toda la info (personas, tareas, restricciones, pesos).
    Returns:
        model: Objeto del problema PuLP.
        X: Variables de decisión (Asignaciones).
        W: Carga de trabajo por persona.
        W_max, W_min: Variables auxiliares para balanceo.
    """
    # Desempaquetado de datos
    people = data['people']
    tasks = data['tasks']
    hours = data['hours']
    D = data['D']  # Disponibilidad (1=Sí, 0=No)
    Q = data['Q']  # Habilitaciones/Cualificaciones (1=Puede hacerla, 0=No)
    R = data['R']  # Requerimientos (cuántas personas se necesitan por tarea/hora)
    F = data['F']  # Asignaciones Fijas/Obligatorias (pre-asignadas por el usuario)
    
    # Parámetros de la Función Objetivo (Pesos)
    alpha = float(data['alpha'])   # Penalización por desbalance (Equidad)
    beta = float(data['beta'])     # Penalización por asignaciones (Continuidad/Carga)
    gamma = float(data['gamma'])   # Penalización por cambios de turno (Switches)
    epsilon = float(data['epsilon']) # Penalización por no cumplir asignaciones fijas (Soft constraint)
    timelimit = int(data['timelimit'])
    
    print("Creando modelo matemático...")
    # Definimos el problema de MINIMIZACIÓN
    model = LpProblem("Staffing", LpMinimize)
    
    # --- VARIABLES DE DECISIÓN ---
    
    # X[i][t][h]: 1 si la persona i hace la tarea t en la hora h, 0 si no.
    X = LpVariable.dicts("X", (people, tasks, hours), cat='Binary')
    
    # W[i]: Carga total de trabajo (horas) de la persona i.
    W = LpVariable.dicts("W", people, lowBound=0, cat='Integer')
    
    # Variables auxiliares para calcular el rango de equidad (Max carga - Min carga)
    W_max = LpVariable("W_max", lowBound=0)
    W_min = LpVariable("W_min", lowBound=0)
    
    # Variables auxiliares para continuidad (Y) y cambios de turno (S)
    hours_minus_last = hours[:-1]
    # Y[i][t][h]: Detecta si alguien trabaja en h y h+1 en la misma tarea (continuidad)
    Y = LpVariable.dicts("Y", (people, tasks, hours_minus_last), cat='Binary')
    
    hours_minus_first = hours[1:]
    # S[i][h]: Detecta si hay un "arranque" de turno (cambio de estado 0 a 1)
    S = LpVariable.dicts("S", (people, hours_minus_first), cat='Binary')
    
    # U[i][t][h]: Variable de holgura para asignaciones fijas (permite violar la fija pagando un precio alto)
    U = LpVariable.dicts("U", (people, tasks, hours), cat='Binary')
    
    # --- FUNCIÓN OBJETIVO ---
    # Minimizamos: Desbalance + (Beta * Continuidad) + (Gamma * Arranques) + (Epsilon * Violación Fijas)
    model += (
        alpha * (W_max - W_min) +
    ...
    # 1. Disponibilidad: No se puede asignar si D[i][h] es 0.
    # Además, una persona solo puede hacer 1 tarea a la vez (suma <= 1).
    for i in people:
        for h in hours:
            model += lpSum(X[i][t][h] for t in tasks) <= D[i][h]
    
    # 2. Requerimientos: La suma de personas en la tarea t a la hora h debe ser igual a R[t][h].
    for t in tasks:
        for h in hours:
            model += lpSum(X[i][t][h] for i in people) == R[t][h]
    
    # 3. Cualificaciones: No asignar si la persona no sabe hacer la tarea (Q[i][t] == 0).
    for i in people:
        for t in tasks:
            for h in hours:
                model += X[i][t][h] <= Q[i][t]
    
    # 4. Cálculo de Carga de Trabajo (W) y definición de Max/Min.
    for i in people:
        model += W[i] == lpSum(X[i][t][h] for t in tasks for h in hours)
        model += W_max >= W[i] # W_max debe ser mayor o igual a cualquier carga
        model += W_min <= W[i] # W_min debe ser menor o igual a cualquier carga
    
    # 5. Lógica de Continuidad (definición de variable Y).
    for i in people:
        for t in tasks:
            for h in hours_minus_last:
                h_next = hours[hours.index(h) + 1]
                # Y >= X_actual + X_siguiente - 1. Solo es 1 si ambos son 1.
                model += Y[i][t][h] >= X[i][t][h] + X[i][t][h_next] - 1
    
    # 6. Lógica de Cambios/Arranques (definición de variable S).
    for i in people:
        for h in hours_minus_first:
            h_prev = hours[hours.index(h) - 1]
            T_ih = lpSum(X[i][t][h] for t in tasks)      # ¿Trabaja ahora?
            T_ih_prev = lpSum(X[i][t][h_prev] for t in tasks) # ¿Trabajaba antes?
            # S >= Actual - Previo. Si pasa de 0 a 1, S=1.
            model += S[i][h] >= T_ih - T_ih_prev
    
    # 7. Asignaciones Fijas (Hard/Soft constraint).
    # U >= Fijo - Asignación Real. Si F=1 y X=0, U=1 (pagando penalización epsilon).
    for i in people:
        for t in tasks:
            for h in hours:
                model += U[i][t][h] >= F[i][t][h] - X[i][t][h]
    
    print(f"Ejecutando solver CBC (limite tiempo: {timelimit}s)...")
    # Resolvemos usando CBC (Coin-or branch and cut), el motor por defecto de PuLP.
...
class StaffingApp:
    ...
    def main(self, page: ft.Page):
        ...
        config_below = ft.Column([
            ...
            ft.Text("4. Parameters", color=self.COLOR_TEXT_HIGHLIGHT, weight="bold", size=20),
    ...
    def execute_bulk_action(self, action_type, matrix_type, key):
        ...
        if matrix_type == 'R':
            # Para requerimientos, resetea a 0
        ...
        # Comprobar estado del primer elemento para decidir si encender o apagar todo
        current_val = 1 
        if action_type == 'row':
    ...
    def gather_data_and_solve(self):
        ...
        # Preparar datos para el Solver (convertir claves string de JSON a int si es necesario)
