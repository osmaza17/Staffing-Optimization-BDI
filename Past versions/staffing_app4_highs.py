...
def solve_model(data):
    """
    Versión ROBUSTA con Diagnóstico de nombres.
    Construye modelo con IDs seguros, resuelve con Highspy e inyecta resultados 
    con verificación explícita de nombres.
    """
    ...
    # 1. CAPA DE SEGURIDAD (Mapeo a ASCII puro)
    # Usamos índices P0, P1... para evitar tildes en el motor matemático
    safe_people = [f"P{i}" for i in range(len(people))]
    safe_tasks =  [f"T{i}" for i in range(len(tasks))]
    
    # 2. VARIABLES DE DECISIÓN (IDs seguros)
    ...
    # 3. RECONSTRUCCIÓN DE DICCIONARIOS (Mapeo Real -> Seguro)
    # Esto permite que la UI use los nombres reales (people[i]) apuntando a las variables seguras
    ...
    # (El resto de variables auxiliares no se usan en la UI, pero las definimos para el solver)
    ...
    # 4. FUNCIÓN OBJETIVO Y RESTRICCIONES (Idéntico a lógica original)
    model += (
        alpha * (W_max - W_min) +
    ...
    try:
        ...
        print(f"Primal Sol Status: {info.primal_solution_status} (2=Feasible)")
        ...
        if status_h == highspy.HighsModelStatus.kOptimal:
            ...
            if has_feasible_sol:
                print("TimeLimit con solución. Forzando Optimal.")
                model.status = LpStatusOptimal
            else:
        ...
        # --- INYECCIÓN DE RESULTADOS CON DIAGNÓSTICO ---
        # Solo inyectamos si hay solución factible, independientemente del status exacto
        # --- INYECCIÓN DE RESULTADOS CON DIAGNÓSTICO ---
        if has_feasible_sol:
            print("--- INYECTANDO RESULTADOS DE HIGHS A PULP ---")
            solution = h.getSolution()
            col_vals = solution.col_value
            num_cols = h.getNumCol()
            
            # Crear mapa {Nombre_Variable: Valor}
            val_map = {}
            for k in range(num_cols):
                # CORRECCIÓN CRÍTICA AQUÍ:
                # getColName devuelve (status, nombre). Desempaquetamos.
                ret = h.getColName(k)
                
                # Verificamos si devuelve tupla o string (depende de la versión de highspy)
                if isinstance(ret, tuple):
                    _, col_name = ret # Ignoramos el status (primer elemento)
                else:
                    col_name = ret # Es un string directo
                
                val_map[col_name] = round(col_vals[k]) # Redondeo
            
            # DIAGNÓSTICO
            first_key = list(val_map.keys())[0]
            print(f"Nombre corregido (ejemplo): '{first_key}' (Tipo: {type(first_key)})")
            ...
            print(f"Variables actualizadas: {matches}. Variables no encontradas: {misses}")
            
            # Verificación final
            total_work_check = sum(v.varValue for v in model.variables() if "X_" in v.name)
            print(f"Suma total de horas asignadas (check): {total_work_check}")

        else:
            print("No se inyectaron resultados (No feasible solution found).")
